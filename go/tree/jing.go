...
//先序遍历
func (node *Node) Getfirstsort(n *Node) {
	if n == nil {
		return
	} else {
		fmt.Printf("%d ", n.value)
		node.Getfirstsort(n.left)
		node.Getfirstsort(n.right)
	}
}

//中序遍历：
func (node *Node) Getmiddlesort(n *Node) {
	if n == nil {
		return
	} else {
		node.Getmiddlesort(n.left)
		fmt.Printf("%d ", n.value)
		node.Getmiddlesort(n.right)
	}
}

//后序遍历：
func (node *Node) Getlastsort(n *Node) {
	if n == nil {
		return
	} else {
		node.Getlastsort(n.left)
		node.Getlastsort(n.right)
		fmt.Printf("%d ", n.value)
	}
}
...
func main() {
	...
	fmt.Printf("\n先序遍历：")
	root.Getfirstsort(root)
	fmt.Printf("\n中序遍历：")
	root.Getmiddlesort(root)
	fmt.Printf("\n后序遍历：")
	root.Getlastsort(root)
}
